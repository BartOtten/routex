...
Extensions are responsible for providing the `on_mount` hook.
`Routex.Extension.LiveViewHooks` is such extension and will inline any Phoenix
Liveview Lifetime callbacks from other extensions. It also attaches a
`handle_param` which in turn assigns a few (helper) values to the connection
and/or socket. This includes the current url.
