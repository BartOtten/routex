defmodule Routex.Extension.SimpleLocale.Extractor do
  ...
  def extract_from_source(%Plug.Conn{} = conn, :cookie, param, _attrs) do
    conn
  ...
  def extract_from_source(%Plug.Conn{} = conn, :session, param, _attrs) do
    case Plug.Conn.get_session(conn, @session_key) do
      nil -> nil
      data -> Map.get(data, param)
    end
  end
  ...
  defp find_first_valid_segment(segments) do
    Enum.find_value(segments, fn segment ->
      validate_locale_value(segment)
    end)
  end
