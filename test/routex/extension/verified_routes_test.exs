...
defmodule Routex.Extension.VerifiedRoutesTest do
  ...
  defp conn_with_endpoint(endpoint \\ Endpoint) do
    conn(:get, "/") |> Plug.Conn.put_private(:phoenix_endpoint, endpoint) |> Plug.Conn.put_private( :routex, %{__order__: [0,1]}) 
  end

  defp socket_with_endpoint(endpoint \\ Endpoint), do: %Phoenix.Socket{endpoint: endpoint, private: %{routex: %{__order__: [0,1]}}}

  def conn_with_script_name(script_name \\ ~w(api)) do
    conn = Plug.Conn.put_private(conn(:get, "/"), :phoenix_endpoint, ScriptName) |> Plug.Conn.put_private( :routex, %{__order__: [0,1]}) 
