defmodule MatchTest do
  ...
  @alt_route_sp %Phoenix.Router.Route{
    hosts: [],
    path: "/:id/:country/producta/edito",
    trailing_slash?: false
   }

	  @alt_route_nl %Phoenix.Router.Route{
    hosts: [],
    path: "/:country/producten/wijzigen/:id",
    trailing_slash?: false
  }

  @uri_matches (for scheme <- ["http", "https", "ftp"],
                    host <- ["localhost"],
                    path <- ["/nl/products/12/edit"],
                    query <- [nil, "k=v"],
                    fragment <- [nil, "top"] do
  ...
  recompose_ast_sp = quote do:  unquote(Match.to_pattern(@alt_route_sp))
	...
	testvar_ast = quote do: unquote(Macro.var(:var, Match)) <> "YES!"
  ...
  test "URL matches Route" do
    for uri <- @uri_matches do
      ...
      assert result.path == "/12/nl/producta/edito"
    end
  end
